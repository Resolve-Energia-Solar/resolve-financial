...
class RequestsEnergyCompany(models.Model):
    ...
    company = models.ForeignKey(
        EnergyCompany, on_delete=models.CASCADE, verbose_name="Distribuidora de Energia"
    )
    project = models.ForeignKey(
        "resolve_crm.Project",
        on_delete=models.CASCADE,
        verbose_name="Projeto",
        related_name="requests_energy_company",
    )
    unit = models.ForeignKey(
        "Units", on_delete=models.CASCADE, verbose_name="Unidade", null=True, blank=True
    )
    type = models.ForeignKey(
        "ResquestType", on_delete=models.CASCADE, verbose_name="Tipo de Solicitação"
    )
    situation = models.ManyToManyField(
        "SituationEnergyCompany", verbose_name="Situação", blank=True
    )
    ...
    status = models.CharField(
        "Status", max_length=2, choices=STATUS_CHOICES, default="S"
    )
    debit_value = models.DecimalField(
        "Valor do Débito", max_digits=10, decimal_places=2, null=True, blank=True
    )
    conclusion_date = models.DateField("Data da Conclusão", null=True, blank=True)
    interim_protocol = models.CharField(
        "Protocolo Provisório", max_length=100, null=True, blank=True
    )
    final_protocol = models.CharField(
        "Protocolo Definitivo", max_length=100, null=True, blank=True
    )
    is_deleted = models.BooleanField("Deletado", default=False)
    created_at = models.DateTimeField("Criado em", auto_now_add=True)
    history = HistoricalRecords()
...
class Units(models.Model):
    ...
    supply_adquance = models.ManyToManyField(
        "SupplyAdequance", verbose_name="Adequação de Fornecimento", blank=True
    )
    main_unit = models.BooleanField("Geradora", default=False)
    unit_percentage = models.DecimalField(
        "Porcentagem de Rateio", max_digits=10, decimal_places=2, null=True, blank=True
    )
    address = models.ForeignKey(
        "accounts.Address",
        on_delete=models.CASCADE,
    ...
    unit_number = models.CharField(
        "Conta contrato", max_length=100, null=True, blank=True
    )
    ...
    account_number = models.CharField(
        "Número do medidor", max_length=100, null=True, blank=True
    )
    bill_file = models.FileField(
        "Arquivo da Fatura", upload_to="units-bills/", null=True, blank=True
    )
    is_deleted = models.BooleanField("Deletado", default=False)
    created_at = models.DateTimeField("Criado em", auto_now_add=True)
    history = HistoricalRecords()
    ...
    def save(self, *args, **kwargs):
        if self.main_unit:
            Units.objects.filter(project=self.project, main_unit=True).exclude(
                id=self.id
            ).update(main_unit=False)
...
class CivilConstruction(models.Model):
    ...
    start_date = models.DateField("Início da Obra", null=True, blank=True)
    end_date = models.DateField("Fim da Obra", null=True, blank=True)
    ...
    is_customer_aware = models.BooleanField("Cliente ciente", default=False)
    repass_value = models.DecimalField(
        "Valor de Repasse", max_digits=10, decimal_places=2, null=True, blank=True
    )
    budget_value = models.DecimalField(
        "Valor de Orçamento", max_digits=10, decimal_places=2, null=True, blank=True
    )
