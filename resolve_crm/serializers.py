...
class SaleSerializer(BaseSerializer):
    ...
    def get_financiers(self, obj):
        financiers = (
            Financier.objects.select_related("address")
            .filter(payment__sale=obj, is_deleted=False)
            .distinct()
        )
        return FinancierSerializer(
            financiers, many=True, fields=("id", "name"), context=self.context
        ).data
    ...
    def get_signature_status(self, obj):
        submissions = list(
            getattr(obj, "all_submissions", obj.contract_submissions.all())
        )
        ...
        if self.instance is None:
            ...
            if "transfer_percentage" not in data:
                if branch and branch.transfer_percentage:
                    ...
                    raise ValidationError(
                        {"transfer_percentage": "Percentual de repasse não cadastrado."}
                    )
    ...
    def _handle_products(self, sale, products=None, commercial_proposal_id=None):
        ...
        if commercial_proposal_id is not None:
            ...
            sale_products_qs = SaleProduct.objects.filter(
                commercial_proposal=proposal, sale__isnull=True
            )

            if not sale_products_qs.exists():
                raise ValidationError(
                    {"detail": "Proposta comercial não possui produtos disponíveis."}
                )
            ...
            new_sale_products = [
                SaleProduct(
                    ...
                    reference_value=prod.reference_value
                    or prod.product_value
                    or Decimal("0"),
        ...
        project_instances = [
            Project(sale=sale, product=sp.product) for sp in sale_products
        ]
        ...
        installment_value = sale.calculate_franchise_installment_value(
            aggregate_reference
        )
    ...
    def create_projects(self, sale, products):
        projects = [Project(sale=sale, product=sp.product) for sp in products]
    ...
    def calculate_total_installment_value(self, sale, products):
        ...
        if difference_value <= 0:
            total_value = (
                reference_value * (transfer_percentage / Decimal("100"))
                - difference_value
            )
        else:
...
class ProjectSerializer(BaseSerializer):
    ...
    is_released_to_installation = serializers.BooleanField(
    ...
    def get_documents_under_analysis(self, obj):
        ...
        return [
            {
                "id": d.id,
                "name": d.document_type.name if d.document_type else None,
                "status": d.status,
            }
            for d in documents
        ]
    ...
    def get_address(self, obj):
        main_unit = (
            obj.main_unit_prefetched[0]
            if hasattr(obj, "main_unit_prefetched") and obj.main_unit_prefetched
            else None
        )
        return (
            AddressSerializer(main_unit.address).data
    ...
    def get_latest_installation(self, obj):
        ...
        inst = Schedule.objects.select_related(
            "address", "schedule_agent", "final_service_opinion_user"
        ).get(pk=inst.pk)
