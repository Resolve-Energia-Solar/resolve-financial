...
class ProjectQuerySet(models.QuerySet):
    ...
    def with_delivery_status(self):
        return (
            ...
            .annotate(
                ...
                has_delivery=Exists(
                    Schedule.objects.filter(
                        project=OuterRef("pk"), service__name__icontains="Entrega"
                    ).values("id")
                ),
    ...
    def with_installation_status(self):
        return (
            self.with_is_released_to_installation()  # Chama o método que já faz várias anotações importantes
            .annotate(
            ...
            .annotate(
                installation_status=Case(
                    # 0) Verificar status de pré-venda e venda cancelada primeiro
                    When(Q(sale__is_pre_sale=True), then=Value("Pré-Venda")),
                    When(
                        Q(sale__status__in=["C", "D"]),
                        then=Value("Venda Cancelada ou Distrato"),
                    ),
                    ...
                    # 4) se não liberado para instalação → bloqueado
                    When(Q(is_released_to_installation=False), then=Value("Bloqueado")),
                    # 5) liberado: está liberado para instalação e entregue
