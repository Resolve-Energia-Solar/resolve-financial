...
class ErrorRequestFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "erro":
            return queryset.filter(responsible_status="A", payment_status="P").filter(
                Q(integration_code__isnull=True) | Q(integration_code="")
            )
        return queryset
...
class FinancialRecordAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        ...
        "department_name",
        "value",
        "responsible__complete_name",
        "requester__complete_name",
    )
    list_filter = (
        "status",
        "responsible_status",
