...
class PaymentViewSet(BaseModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        sale_marketing_campaign = request.query_params.get(
            "sale_marketing_campaign", None
        )
        sale_branch = request.query_params.get("sale_branch", None)
        principal_final_service_opinion = request.query_params.get(
            "principal_final_service_opinion__in", None
        )
        final_service_opinions = request.query_params.get(
            "final_service_opinions__in", None
        )

        if principal_final_service_opinion:
            principal_final_service_opinion = principal_final_service_opinion.split(",")
            queryset = queryset.filter(
                sale__projects__inspection__final_service_opinion__id__in=principal_final_service_opinion
            )

        if final_service_opinions:
            final_service_opinions = final_service_opinions.split(",")
            queryset = queryset.filter(
                sale__projects__field_services__final_service_opinion__id__in=final_service_opinions
            )
        ...
        if sale_marketing_campaign:
            queryset = queryset.filter(
                sale__marketing_campaign__id__in=sale_marketing_campaign
            )

        if sale_payment_status:
            sale_payment_status_list = sale_payment_status.split(",")
            queryset = queryset.filter(
                sale__payment_status__in=sale_payment_status_list
            )
    ...
    def indicators(self, request, *args, **kwargs):
        ...
        installments_indicators = qs.aggregate(
            overdue_installments_count=Count(
                ...
                filter=Q(
                    ...
                    installments__due_date__lte=timezone.now(),
                ),
            ),
            ...
            on_time_installments_count=Count(
                ...
                filter=Q(
                    installments__is_paid=False,
                    installments__due_date__gt=timezone.now(),
                ),
            ),
            ...
            paid_installments_count=Count(
                "installments", filter=Q(installments__is_paid=True)
            ),
        ...
        consistency_indicators = qs_consistency.aggregate(
            ...
            inconsistent_payments_value=Coalesce(
                Sum(
                    "value", filter=~Q(is_consistent=True), output_field=DecimalField()
                ),
                Value(0, output_field=DecimalField()),
            ),
...
class FinancialRecordViewSet(BaseModelViewSet):
    ...
    def get_queryset(self):
        ...
        DEPT_SET_3_IDS = [
            ...
            13,
            20,
        ]  # Vistoria, Financeiro do CO, Obras, Instalação
        ...
        condition_d = user.has_perm(
            "financial.view_all_department_payable_financial_records"
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        instance = self.get_object()
        generate_pdf = (
            request.query_params.get("generate_pdf", "false").lower() == "true"
        )
        ...
        if generate_pdf:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="solicitacao_{instance.protocol}.pdf"'
            )
...
class OmieIntegrationView(APIView):
    ...
    def post(self, request):
        if not self.OMIE_API_URL or not self.OMIE_ACESSKEY or not self.OMIE_ACESSTOKEN:
            return Response(
                {
                    "error": "API URL, AcessKey and AccessToken not set in environment variables"
                },
                status=500,
            )
    ...
    def list_departments(self):
        ...
        if response.status_code == 200:
            ...
            active_departments = [
                dept for dept in departments if dept.get("inativo") == "N"
            ]
            return Response(active_departments)
        else:
            return Response(
                {"error": "Failed to fetch data from Omie API"},
    ...
    def list_customers(self, filter):
        ...
        if filter.isdigit():
            filter = {"cnpj_cpf": filter, "inativo": "N"}
        else:
            filter = {"razao_social": filter, "inativo": "N"}
    ...
    def create_customer(self, customer):
        ...
        if response.status_code == 200:
            ...
            return Response(
                {
                    "error": "Failed to create customer in Omie",
        ...
        try:
            ...
            if "razao_social" in result and "cnpj_cpf" in result:
                return {
                    ...
                    "cnpj_cpf": result.get("cnpj_cpf"),
                }
            else:
                logger.error(f"Fornecedor sem documento ou nome: {result}")
                return Response(
                    {"error": "Supplier without document or name"}, status=500
                )
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to fetch supplier from Omie: {e}")
            return Response(
                {"error": "Failed to fetch supplier from Omie", "details": str(e)},
                status=500,
            )

    def create_payment_request(
        self, financial_record, manager_status="Aprovado", manager_note=None
    ):
        if manager_status == "Aprovado":
                ...
                "param": [
                    {
                        ...
                        "data_vencimento": financial_record.due_date.strftime(
                            "%d/%m/%Y"
                        ),
                        ...
                        "numero_documento_fiscal": (
                            ...
                            if financial_record.invoice_number
                            else ""
                        ),
                        "data_emissao": financial_record.service_date.strftime(
                            "%d/%m/%Y"
                        ),
            ...
            response = requests.post(url, headers=headers, json=data)
            if response.status_code == 200:
                financial_record.integration_code = response.json().get(
                    "codigo_lancamento_omie", None
                )
                financial_record.save()
            else:
                logger.error(
                    f"Failed to create financial record in Omie: {response.json()}"
                )

        financial_record.status = "E"
        financial_record.responsible_status = (
            "A" if manager_status == "Aprovado" else "R"
        )
        ...
        if manager_status == "Aprovado":
            ...
            return Response(
                {"message": "Financial record updated without sending to Omie"},
                status=200,
            )
    ...
    def post(self, request):
        ...
        for row in rows:
            ...
            if not financial_record_id or not manager_status:
                return Response(
                    {"error": "Missing financial_record_id or manager_status"},
                    status=400,
                )
            ...
            try:
                ...
                return Response(
                    {
                        "error": f"FinancialRecord with id {financial_record_id} does not exist"
                    },
                    status=404,
                )
            ...
            if financial_record.responsible_status != "P":
                return Response(
                    {
                        "error": f"FinancialRecord with id {financial_record_id} is not pending approval"
                    },
                    status=400,
                )
            ...
            try:
                ...
                if financial_record.due_date <= now:
                    ...
                    financial_record.save()
                    logger.info(
                        f"Due date for financial record {financial_record.protocol} updated to {financial_record.due_date}"
                    )
    ...
    def post(self, request):
        ...
        if isinstance(event, dict):
            ...
            if len(conta_a_pagar) > 0:
                financial_record_id = conta_a_pagar[0].get(
                    "codigo_lancamento_integracao", None
                )
...
class SendFinancialRecordsToOmieView(APIView):
    def post(self, request):
