...
class Materials(models.Model):
    ...
    price = models.DecimalField(
        "Preço", max_digits=20, decimal_places=3, default=0, null=False, blank=False
    )
    ...
    created_at = models.DateTimeField(
        "Criado em", auto_now_add=True, null=True, blank=True
    )
...
class ProductMaterials(models.Model):
    product = models.ForeignKey(
        "logistics.Product",
        on_delete=models.CASCADE,
        verbose_name="product de Energia Solar",
        null=True,
        blank=True,
    ...
    amount = models.DecimalField(
        "Quantidade", max_digits=20, decimal_places=6, default=0, null=True, blank=True
    )
    is_deleted = models.BooleanField("Deletado", default=False, null=True, blank=True)
    created_at = models.DateTimeField(
        "Criado em", auto_now_add=True, null=True, blank=True
...
class Product(models.Model):
    ...
    materials = models.ManyToManyField(
        Materials, through="logistics.ProductMaterials", verbose_name="Materiais"
    )
    roof_type = models.ForeignKey(
        "field_services.RoofType",
        on_delete=models.CASCADE,
    ...
    params = models.DecimalField(
        "Parâmetros", max_digits=6, decimal_places=2, null=True, blank=True
    )
...
class ProjectMaterials(models.Model):
    project = models.ForeignKey(
        "resolve_crm.Project", on_delete=models.CASCADE, verbose_name="Projeto"
    )
    material = models.ForeignKey(
        Materials, on_delete=models.CASCADE, verbose_name="Material"
    )
    amount = models.DecimalField(
        "Quantidade", max_digits=20, decimal_places=6, default=0
    )
    material_class = models.CharField(
        "Classe do Material",
        max_length=2,
        choices=[("K", "Kit"), ("P", "Padrão")],
        null=True,
        blank=True,
    )
    is_exit = models.BooleanField("Saída", default=False)
    serial_number = models.CharField(
        "Número de Série", max_length=50, null=True, blank=True
    )
    is_deleted = models.BooleanField("Deletado", default=False)
...
class Purchase(models.Model):
    ...
    supplier = models.ForeignKey(
        "accounts.User", on_delete=models.CASCADE, verbose_name="Fornecedor"
    )
    ...
    purchase_value = models.DecimalField(
        "Valor da Compra", max_digits=20, decimal_places=2, default=0
    )
    delivery_forecast = models.DateField("Previsão de Entrega", null=True, blank=True)
    delivery_number = models.CharField(
        "Número de Entrega", max_length=50, null=True, blank=True
    )
    is_deleted = models.BooleanField("Deletado", default=False)
